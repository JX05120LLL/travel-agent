...
def _duration_to_minutes(duration_text: str | None) -> int:
...
def _rank_rail_candidates(candidates: list["RailTripOption"]) -> list["RailTripOption"]:
    """Prefer G/D/C trains, then shorter duration, while keeping stable tie order."""
    return sorted(
        candidates,
        key=lambda item: (
            0 if _is_high_speed_train(item.train_no) else 1,
            _duration_to_minutes(item.duration_text),
        ),
    )
...
class JisuApiTrainProvider:
    ...
    def __init__(self) -> None:
        ...
        self.timeout_seconds = float(os.getenv("JISU_TRAIN_TIMEOUT_SECONDS", "45") or "45")
...
class MCP12306Provider:
    ...
    def __init__(self) -> None:
        ...
        self.timeout_seconds = float(os.getenv("MCP_12306_TIMEOUT_SECONDS", "45") or "45")
    ...
    def search_trips(self, query: RailTripQuery) -> RailTripResult:
        ...
        candidates = _rank_rail_candidates(candidates)
    ...
    def search_transfers(self, query: RailTripQuery) -> dict[str, Any]:
        """Query one-transfer plans from the 12306 MCP server."""
        if not self.endpoint:
            raise ServiceConfigError("MCP 12306 查询未配置，无法查询中转方案。")
        ...
        def _load() -> dict[str, Any]:
            with httpx.Client(timeout=self.timeout_seconds) as client:
                ...
                try:
                    ...
                    if not text:
                        raise ServiceIntegrationError("MCP 12306 query-transfer 未返回文本内容。")
                    ...
                    if not payload.get("success"):
                        ...
                        raise ServiceIntegrationError(f"MCP 12306 query-transfer 失败：{message}")
                    return payload
                finally:
                    try:
...
class TuniuFreeApiProvider:
    ...
    def __init__(self) -> None:
        ...
        self.timeout_seconds = float(os.getenv("TUNIU_TRAIN_TIMEOUT_SECONDS", "45") or "45")
