...
TRAVEL_PLANNING_EXPANSION_KEYWORDS = (
    "旅游",
    "旅行",
    "行程",
    "攻略",
...
def looks_like_rail_only_request(text: str) -> bool:
    """识别只查跨城铁路信息的请求，避免误扩展成完整旅行规划。"""
    normalized = (text or "").strip()
    if not normalized:
        return False
    if not any(keyword in normalized for keyword in RAIL_ONLY_KEYWORDS):
        return False
    if any(keyword in normalized for keyword in TRAVEL_PLANNING_EXPANSION_KEYWORDS):
        return False
    return True


def _build_rail_only_section(user_input: str) -> str | None:
    """为纯火车票查询注入强约束，避免模型顺手调用天气/RAG/攻略工具。"""
    if not looks_like_rail_only_request(user_input):
        return None
    return (
        "【本轮工具边界】\n"
        "用户本轮只是在查询火车票/高铁/动车/车次/余票/票价信息。"
        "请只围绕铁路到达信息回答；如需工具，本轮最多调用 plan_12306_arrival。\n"
        "不要自动扩展成旅行攻略或一条龙行程；不要调用 get_weather、"
        "retrieve_local_knowledge、search_travel_info、酒店、美食、景点、地图类工具。\n"
        "最终回答只需整理车次、出发/到达站、时间、耗时、票价/余票参考和12306官方购票提醒。"
    )
...
def _build_one_stop_planning_section(
    *,
    ...
    return (
        ...
        "本轮按完整一条龙服务推进：可以查询到达交通、天气、本地知识/联网搜索、"
        "景点/酒店/美食/地图信息；信息足够后请输出成品行程单，不要反复搜索同类信息。\n"
        "地图预览可作为完整攻略的一部分；专属地图 create_amap_personal_map 只在用户明确要求"
        "专属地图、导航或发到手机时再调用。\n"
...
class SessionService:
    ...
    def apply_user_input(
        ...
        *,
        ...
        rail_only_section = _build_rail_only_section(user_input)
        if rail_only_section and rail_only_section not in result.extra_sections:
            result.extra_sections.append(rail_only_section)

        one_stop_section = None if rail_only_section else _build_one_stop_planning_section(
            route=route,
            user_input=user_input,
        )
