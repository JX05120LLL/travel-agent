...
class PlanOptionDestination(Base):
    """候选方案中的目的地明细。"""

    __tablename__ = "plan_option_destinations"

    __table_args__ = (
        UniqueConstraint(
            "plan_option_id",
            "sequence_no",
            name="uq_plan_option_destinations_sequence",
        ),
        CheckConstraint(
            "stay_days IS NULL OR stay_days > 0",
            name="chk_plan_option_destinations_stay_days",
        ),
    )
    ...
    plan_option: Mapped["PlanOption"] = relationship(back_populates="destinations")
